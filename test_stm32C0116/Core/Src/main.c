...
void Set_PWM_From_Analog(uint32_t adc_value);
void Set_LED_From_Analog(uint32_t adc_value);
...
int main(void)
{
  ...
  MX_I2C1_Init();
  /* USER CODE BEGIN 2 */
  MX_TIM3_Init();								// why not autogenerated by cubemx ???
...
// Function to map ADC value (0-4095) to PWM duty cycle (0-100%)
void Set_PWM_From_Analog(uint32_t adc_value) {
    // Map ADC value (0-4095) to PWM pulse (0-TIM_ARR)
    uint32_t pwm_pulse = (adc_value * htim3.Instance->ARR) / 4095;
